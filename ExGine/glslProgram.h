...
void glslProgram::sendUniform(const std::string &name, float *m, bool transp=false, int size=4)
{
	switch(size)
	{
		case 2:
		...
		case 3:
			...
			break;
		case 4:
			glUniformMatrix4fv(glGetUniformLocation(handle,(const GLchar*)name.c_str()), 1, transp, m);
			break;
	}
}
