...
{
public:
	Scene(){/*testGUI = new Gui();*/currentShaderID = 0;keyboard = new Keyboard();fbo = NULL;offset = blink= doFPS = pickMode=toFbo=false;root=current=0;full=0;lightToggle=toggle=count=0;worldRot.x = 0;worldRot.y = 0;worldRot.z = 0;worldPos.x = 0;worldPos.y = 0;worldPos.z = -10;};
	~Scene(){delete [] root; delete [] current;};
	void saveScene(string filename);
	bool loadScene(const char* filename);
	void draw(void);
	long addModel(Model* _model, Material* _mat, Animation* _ani);
	void addAnimToModel(long ID, Animation* _ani){findMNode(ID)->ani = _ani;};
	...
	long getCurrentModel(){return toggle;};
		
	void addShader(glslProgram* shader);
	void deleteShader();
	
	void moveCamera(Vector<Scalar> pos);
	...
	void rotateCamera(Vector<Scalar> rot);
	...
	Vector<Scalar> getCameraPos();
	Vector<Scalar> getCameraRot();
	
	void moveModel(long modelID, Vector<Scalar> pos);
	...
	void rotateModel(long modelID, Vector<Scalar> rot);
	...
	void scaleModel(long modelID, Vector<Scalar> scale);
	...
	Vector<Scalar> getModelPos(long modelID);
	Vector<Scalar> getModelRot(long modelID);
	...
	void moveLight(long lightID, Vector<Scalar> pos);
	...
	void rotateLight(long lightID, Vector<Scalar> rot);
	...
	//becareful with the return value from these, if the ID doesn't exsist there could be problems, I'll work it out later -Matt
	Vector<Scalar> getLightPos(long lightID);
	Vector<Scalar> getLightRot(long lightID);
	void empty();
	
	long getModelCount(){return count;};
	long getLightCount(){return lcount;};
	Model* getModel(long modelID);
	Material* getMaterial(long modelID);
protected:
	...
	Vector<Scalar> worldRot;
	Vector<Scalar> worldPos;
private:
	MNode* findMNode(long ID){
		current = root;
...
{  
	...
	if(first)
	{
		...
		string log;
		shaders[0]->GetProgramLog(log);
		std::clog<<"Fraglight Shader Log:\n"<<log<<std::endl;
		shaders[1]->GetProgramLog(log);
		std::clog<<"NMB Shader Log:\n"<<log<<std::endl;
		shaders[2]->GetProgramLog(log);
		std::clog<<"CSM Shader Log:\n"<<log<<std::endl;
		shaders[3]->GetProgramLog(log);
...
//			current->mat->addToModel();
			if(pickMode)
			{
				current->model->draw(0,false,current->ani);
			}
			else
			{
				if(offset&&current->ID==toggle)
				{
					...
					if(currentShaderID == 0)
						current->model->draw(NULL,true,current->ani);
					else
						current->model->draw(shaders[currentShaderID-1],true,current->ani);
...
long Scene::addModel(Model* _model,Material* _mat, Animation* _ani = NULL)
{
	long tempID=-1;
	bool done=false;
  if(count == 0||root==0)
...
void Scene::KB(unsigned char key, int x, int y)
{
	...
	if(!keyboard->getKey(KEYBOARDINPUT_MODE))
	{
		if(mod == GLUT_ACTIVE_SHIFT)
			keyboard->setKeyDown(SHIFT_KEY);
		else
			keyboard->setKeyUp(SHIFT_KEY);;
			
		if(mod == GLUT_ACTIVE_ALT)
			keyboard->setKeyDown(ALT_KEY);
		else
			keyboard->setKeyUp(ALT_KEY);
		
		if(mod == GLUT_ACTIVE_CTRL)
			keyboard->setKeyDown(CTRL_KEY);
		else
			keyboard->setKeyUp(CTRL_KEY);
		...
		switch(key)
		{
			...
			case 'm':
			case 'M':
				findMNode(toggle)->ani->start();
				break;
					
			case 'a':
			case 'A':
				rotateModelInc(toggle,'y',-1);
				if(keyboard->getKey(SHIFT_KEY))
					rotateModelInc(toggle,'y',-1);
				break;	
			...
			case 'i':
			case 'I':
				keyboard->setKeyDown(KEYBOARDINPUT_MODE);
				keyboard->setKeyDown(LOADMODEL_MODE);
				break;
			case 'o':
			case 'O':
				keyboard->setKeyDown(KEYBOARDINPUT_MODE);
				keyboard->setKeyDown(SAVELEVEL_MODE);
				break;
			case 'p':
			case 'P':
				keyboard->setKeyDown(KEYBOARDINPUT_MODE);
				keyboard->setKeyDown(LOADLEVEL_MODE);
				break;
			case '-':
				break;
			case '_':
			...
			case 'V':
				keyboard->setKeyDown(KEYBOARDINPUT_MODE);
				keyboard->setKeyDown(LOADSHADER_MODE);
				firstName =true;
				break;
		}
	}
	else
	{
		...
		{
			case 13: //ENTER
				if(keyboard->getKey(SAVELEVEL_MODE))
				{
					saveScene(temp);
				...
				if(keyboard->getKey(LOADMODEL_MODE))
				{
					tmat = new Material();
					tempm = new StaticModel(temp);
					addModel(tempm,tmat);	
					#pragma omp parallel for
					for(int i = KEYBOARDINPUT_MODE; i<=KEYBOARDINPUT_MODE+MAXNUM_KEYBOARD_MODES;i++)
						keyboard->setKeyUp(i);
				...
				if(keyboard->getKey(LOADLEVEL_MODE))
				{
				...
				if(keyboard->getKey(LOADSHADER_MODE))
				{
					if (firstName)
					{
						temp2 = temp;
	...
	switch(key)
	{
		...
		case GLUT_KEY_F8:
			...
			if(offset)
				...
				std::clog<<"SELECTION OFFSET OFF"<<std::endl;
			break;
		case GLUT_KEY_F9:
			...
			if(offset)
				...
				std::clog<<"SELECTION BLINK OFF"<<std::endl;
			break;
	}
}
	...
	switch(button)
	{
		case GLUT_MIDDLE_BUTTON:
			if (state == GLUT_DOWN)
			{
				...
				mouseX = x;
				mouseY = y;
				keyboard->setKeyDown(MIDDLE_MOUSE);
			...
			if (state == GLUT_UP)
			{
				...
				if(mod == GLUT_ACTIVE_CTRL)
					...
					keyboard->setKeyUp(CTRL_KEY);
				keyboard->setKeyUp(MIDDLE_MOUSE);
			}
			break;
...
void Scene::moveModel(long modelID, Vector<Scalar> pos)
{
...
void Scene::rotateModel(long modelID, Vector<Scalar> rot)
{
	//check for valid modelID
	if(modelID<0||modelID>=count)
		return;
...
void Scene::scaleModel(long modelID, Vector<Scalar> scale)
{
	//check for valid modelID
	if(modelID<0||modelID>=count)
		return;
...
Vector<Scalar> Scene::getModelPos(long modelID)
{
		//check for valid modelID
	if(modelID<0||modelID>=count)
	{
...
Vector<Scalar> Scene::getModelRot(long modelID)
{
		//check for valid modelID
	if(modelID<0||modelID>=count)
	{
...
void Scene::moveLight(long lightID, Vector<Scalar> pos)
{
...
void Scene::rotateLight(long lightID, Vector<Scalar> rot)
{
	//check for valid lightID
	if(lightID<0||lightID>=lcount)
		return;
...
Vector<Scalar> Scene::getLightPos(long lightID)
{
		//check for valid lightID
	if(lightID<0||lightID>=lcount)
	{
...
Vector<Scalar> Scene::getLightRot(long lightID)
{
		//check for valid lightID
	if(lightID<0||lightID>=lcount)
	{
...
void Scene::moveCamera(Vector<Scalar> pos)
{
	worldPos.x = pos.x;
	worldPos.y = pos.y;
	worldPos.z = pos.z;
...
void Scene::rotateCamera(Vector<Scalar> rot)
{
	worldRot.x=rot.x;
	worldRot.y=rot.y;
	worldRot.z=rot.z;
...
Vector<Scalar> Scene::getCameraPos()
{
	return worldPos;
}

Vector<Scalar> Scene::getCameraRot()
{
	return worldRot;
}
...
{	
	...
	if(keyboard->getKey(MIDDLE_MOUSE))
	{
		if(!keyboard->getKey(ALT_KEY)&&!keyboard->getKey(CTRL_KEY))
		{
			if(keyboard->getKey(SHIFT_KEY))
...
void Scene::PassMouseMotion(int x, int y)
{
	...
	if(keyboard->getKey(KEY_MODEL_MOVE_X))
	{
		if(!trip)
		{
			mouseX = x;
	...
	else if(keyboard->getKey(KEY_MODEL_MOVE_Y))
	{
		if(!trip)
		{
			mouseX = x;
	...
	else if(keyboard->getKey(KEY_MODEL_MOVE_Z))
	{
		if(!trip)
		{
			mouseX = x;
	...
	else if(keyboard->getKey(KEY_MODEL_ROTATE_X))
	{
		if(!trip)
		{
			mouseX = x;
	...
	else if(keyboard->getKey(KEY_MODEL_ROTATE_Y))
	{
		if(!trip)
		{
			mouseX = x;
	...
	else if(keyboard->getKey(KEY_MODEL_ROTATE_Z))
	{
		if(!trip)
		{
			mouseX = x;
	...
	else if(keyboard->getKey(KEY_MODEL_SCALE))
	{
		if(!trip)
		{
			mouseX = x;
	...
	else if(keyboard->getKey(KEY_MODEL_SCALE_X))
	{
		if(!trip)
		{
			mouseX = x;
	...
	else if(keyboard->getKey(KEY_MODEL_SCALE_Y))
	{
		if(!trip)
		{
			mouseX = x;
	...
	else if(keyboard->getKey(KEY_MODEL_SCALE_Z))
	{
		if(!trip)
		{
			mouseX = x;
	...
	else if(keyboard->getKey(KEY_LIGHT_MOVE_X))
	{
		if(!trip)
		{
			mouseX = x;
	...
	else if(keyboard->getKey(KEY_LIGHT_MOVE_Y))
	{
		if(!trip)
		{
			mouseX = x;
	...
	else if(keyboard->getKey(KEY_LIGHT_MOVE_Z))
	{
		if(!trip)
		{
			mouseX = x;
	...
	else if(keyboard->getKey(KEY_LIGHT_ROTATE_X))
	{
		if(!trip)
		{
			mouseX = x;
	...
	else if(keyboard->getKey(KEY_LIGHT_ROTATE_Y))
	{
		if(!trip)
		{
			mouseX = x;
	...
	else if(keyboard->getKey(KEY_LIGHT_ROTATE_Z))
	{
		if(!trip)
		{
			mouseX = x;
