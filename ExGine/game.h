...
//  }
  ...
  mFile* file;
  Animation* ani = new Animation("data/test.ea");
...
{  
	...
	for(int i=1;i<argc;i++)
	{
		...
		if (file->isStatic())
			model = new StaticModel(file);
		else
			model = new AModel(file);
  		currentScene->addModel(model, mat, ani);//*/
  	}
}
else
{  
  model = new StaticModel("data/ship.em");
  currentScene->addModel(model, mat, ani);
 }//*/
 clog<<"before:"<<light<<endl;
