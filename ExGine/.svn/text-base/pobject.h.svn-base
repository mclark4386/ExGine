...
{
public:
	PObject(){time.start();};///< default base constructer
...
void PObject::runVelocity()
{
	...
	Scalar deltaTime = Scalar(time.delta());
	if(deltaTime>CHECK_INTRIVAL)
	{
		time.reset();
		deltaTime /= 1000;
		Vector<Scalar> tempVec = velocity * deltaTime;
		PoM += tempVec;
	}
}
...
Scalar POSphere::detectCollision(PObject* object)
{
	...
	if (temp[0] > 0.999&&temp[0]<1.001)//if a Scalar "equal" to 1.0 it's a sphere too
	{
		...
		//std::clog<<"SvS r1:"<<radius<<" r2:"<<temp[1]<<" dist:"<<dist<<std::endl;
